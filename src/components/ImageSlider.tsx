...
const ImageSlider = () => {
  ...
  const slides = [
    ...
    },
    {
      id: 4,
      image: "/placeholder.svg",
      title: "Funded by Banbeis",
      subtitle: "Smart Fish Farm Monitoring and Management System",
      description: "IoT and ML powered system to enhance productivity and efficiency in aquaculture.",
      gradient: "from-yellow-600 via-orange-500 to-red-500"
    },
    {
      id: 5,
      image: "/placeholder.svg",
      title: "AI-Driven Fish Farming",
      subtitle: "Smart Monitoring & Feeding",
      description: "Sensors connected to Raspberry Pi track water conditions and automate feeding via ML.",
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`transition-all duration-300 rounded-full border border-white/30 ${
              index === currentSlide ? 'bg-white w-8 h-2' : 'bg-white/50 hover:bg-white/70 w-2 h-2 hover:scale-125'
            }`}
          />
        ))}
      </div>
